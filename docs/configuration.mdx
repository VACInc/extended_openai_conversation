...
<AccordionGroup>
  ...
  <Accordion title="Model">
    ...
    **Default**: `gpt-5-mini`
    ...
    Optional request overrides can be appended as a query string. Example:
    - `openclaw:main?session_key=agent:main:openai:ha-spark&user=ha-voice`

    Supported query parameters:
    - `user` → forwarded as the standard OpenAI `user` field
    - `session_key`, `openclaw_session_key`, `session` → forwarded as `x-openclaw-session-key`
  </Accordion>
